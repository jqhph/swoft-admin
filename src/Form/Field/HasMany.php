...
use Swoft\Admin\Form\NestedForm;
...
class HasMany extends Field
{
    ...
    public function getValidatorData(array &$input)
    {
        ...
        return [&$newRules, &$this->validationMessages, &$attributes];
    }
    ...
    protected function buildRelatedForms()
    {
         ...
         */
        if ($values = old_input($this->column)) {
            ...
            $values = is_string($this->value) ? json_decode($this->value, true) : $this->value;
            $values = is_array($values) ? $values : [];

            foreach ($values as $data) {
                $key = array_get($data, $this->getKeyName());
